import ExperienceData from '@/components/Experience/Experience'; // Renamed to avoid conflict
import { StaticImport } from 'next/dist/shared/lib/get-img-props';
import Image from 'next/image';
import React from 'react';

// Define a type for the items in the ExperienceData array
          ...
          {ExperienceData.map((item: ExperienceItem, index: number) => (
            ...
            >
              <div>
                <Image
                  src={item.src}
                  alt={typeof item.label === 'string' ? item.label : 'Experience item'} // Fallback alt text if label is not a string
                  width={120}
                  height={120}
